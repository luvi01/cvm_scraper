...
class PdfDownloader:
    ...
    async def download_pdf_file(self, session, url, date):
        parsed_url = urlparse(url)
        params = parse_qs(parsed_url.query)
        local_filename = f"{params['CodigoInstituicao'][0]}_{params['numProtocolo'][0]}_{params['numSequencia'][0]}_{params['numVersao'][0]}___{date}.pdf"
    ...
    async def download_pdfs_files(self, urls_date):
        async with aiohttp.ClientSession() as session:
            ...
            for url, date in urls_date:
                try:
                    file = await asyncio.wait_for(self.download_pdf_file(session, url, date), timeout=60.0)  # Set your desired timeout here
